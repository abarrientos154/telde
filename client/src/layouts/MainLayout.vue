    ...
    <q-page-container class="q-mb-md">
      ...
      <q-footer elevated v-if="login">
        <div class="bg-blue-14 shadow-2 full-width row justify-around" >
          ...
          <div class="row items-center"><q-btn :icon="rol === 3 ? 'monetization_on' : 'store'" :label="rol === 3 ? 'Monedero' : 'Tiendas'" color="white" flat stack dense no-caps size="md" /></div>
          <div class="row items-center"><q-btn :icon="rol === 3 ? 'description' : 'favorite'" :label="rol === 3 ? 'Estadísticas' : 'Favoritos'" color="white" flat stack dense no-caps size="md" @click="rol === 2 ? $router.push('/tiendas_favoritas') : ''" /></div>
